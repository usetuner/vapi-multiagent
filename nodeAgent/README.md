# Node agent (Express / TypeScript)

A transient VAPI voice-agent server in TypeScript. On each inbound call it builds
a personalized assistant from a (fake) CRM lookup, executes the agent's tools
mid-call, and on hang-up forwards the call — with VAPI call-log enrichment — to
[Tuner](https://usetuner.ai).

This is the line-for-line counterpart of [`../pythonAgent`](../pythonAgent). Same
...
| File | Description |
|------|-------------|
| `vapi_server.ts` | The webhook server (transient agent + tools + Tuner forwarding) |
| `send_to_tuner.ts` | Tuner client + VAPI call-log enrichment (no npm deps; uses built-in `fetch`) |
...
Edit the config block at the top of `send_to_tuner.ts`:
...
PORT=3000 npm run dev    # or override the port
```
...
- Transient agents are returned inline in the webhook response, so **no VAPI API
  key is required**.
- Swap `fakeCrmLookup` for a real HTTP/DB call to make this production-shaped.
